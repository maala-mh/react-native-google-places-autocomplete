...
> **Provenance.** This is a fork of
> [FaridSafi/react-native-google-places-autocomplete](https://github.com/FaridSafi/react-native-google-places-autocomplete),
> MIT licensed, copyright (c) 2015 Farid from Safi — see [LICENSE](LICENSE). Upstream
> authorship is retained in `package.json`.
>
> It carries one change on top of upstream: a `GooglePlacesDetailsQuery` prop, spread
> into the place-details request so extra query parameters can be passed through.
>
> Commit history was not carried over, so this repo starts from a single squashed
> commit. That commit is content-identical to upstream commit `e133409`; it is not
> original work by this repo's owner.
